...
public class LogicalTimeTest {
    ...
    void convert(CharSequence v, LocalTime expected) throws Exception {
        AvroConverterTest.Utils.oneField(
            v, expected, LogicalTypes.timeMicros().addToSchema(Schema.create(Schema.Type.LONG)),
            false
        );
        AvroConverterTest.Utils.oneField(
            v, expected, LogicalTypes.timeMillis().addToSchema(Schema.create(Schema.Type.INT)),
            false
        );
    }
    ...
    void failed(Object v) {
        AvroConverterTest.Utils.oneFieldFailed(
            v, LogicalTypes.timeMicros().addToSchema(Schema.create(Schema.Type.LONG)),
            false
        );
    }
}
