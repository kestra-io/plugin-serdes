...
class ProtobufToIonTest {
        ...
        static void setup() throws Exception {
                ...
                // Load descriptor
                FileDescriptorSet descriptorV10Set = FileDescriptorSet
                                .parseFrom(new FileInputStream(descriptorV10File));
                Descriptors.Descriptor descriptorV10 = findMessageDescriptor(descriptorV10Set, TYPE_NAME);
                if (descriptorV10 == null) {
                        throw new IllegalArgumentException("Message type not found in descriptor: " + TYPE_NAME);
                }
                FileDescriptorSet descriptorV11Set = FileDescriptorSet
                                .parseFrom(new FileInputStream(descriptorV11File));
                ...
                Map<String, Object> product1 = Map.of("product_id", "1", "product_name", "streamline", "brand",
                                "gomez", "description", "streamline");
                Map<String, Object> product2 = Map.of("product_id", "2", "product_name", "turn-key", "brand",
                                "rodriguez", "description", "turn-key");
        ...
        void testDelimitedMessages() throws Exception {
                ...
                for (URI descriptorUri : new URI[] { descriptorV10Uri, descriptorV11Uri }) {
                        for (URI source : new URI[] { sourceV10, sourceV11 }) {
                                var task = ProtobufToIon.builder()
                                                .id("protobuf-to-ion-delimited")
                                                .type(ProtobufToIon.class.getName())
                                                .from(Property.ofValue(source.toString()))
                                                .descriptorFile(Property.ofValue(descriptorUri.toString())) // now a URI
                                                .typeName(Property.ofValue(TYPE_NAME))
                                                .delimited(Property.ofValue(true))
                                                .build();
        ...
        void testErrorOnUnknwonMessage() throws Exception {
                ...
                var task = ProtobufToIon.builder()
                                ...
                                .typeName(Property.ofValue(TYPE_NAME))
                                .delimited(Property.ofValue(true))
                                .errorOnUnknownFields(Property.ofValue(true))
                                .build();
        ...
        private static Descriptors.Descriptor findMessageDescriptor(FileDescriptorSet descriptorSet, String typeName)
        ...
        private static void createSingleFile(File output, Descriptors.Descriptor descriptor, Map<String, Object> fields)
                        throws IOException {
